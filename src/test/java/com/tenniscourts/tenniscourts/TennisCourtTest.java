...
class TennisCourtTest {
    ...
    void shouldCreateATennisCourt() throws Exception {
        TennisCourtDTO dto = TennisCourtDTO
                .builder().name("Some Tennis Court").tennisCourtSchedules(new ArrayList<>()).build();
    ...
    void shouldUpdateATennisCourt() throws Exception {
        TennisCourtDTO dto = TennisCourtDTO
                .builder().name("A new Tennis Court").id(1L).tennisCourtSchedules(new ArrayList<>()).build();
        mockMvc.perform(post("/tennis-courts").contentType("application/json")
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isCreated());
    }
